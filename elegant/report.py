...
def make_system_schematic(curves, grid, initial_fontsize, save=False,
                          filepath=None, show=False, ext='pdf'):
    """Plots the system schematic
    ...
    initial_fontsize: int
        Will try to annotate the load-flow solution with this fontsize.
        If overlaps happen, will keep trying with smaller texts.
    save: bool, optional
        Whether to call plt.savefig.
    filepath: string, optional
        If save is True, will save the figure to filepath.
    show: bool, optional
        Whether to show the plot.
    ext: string, optional
        File extension to save figure if save is True, defaults to 'pdf'.
    """
    fs = initial_fontsize
    ax = plt.gca()
...
def safe_repr(val, is_tex=True):
    if is_tex:
        if np.abs(val) < 1e6:
            if isinstance(val, complex):
                return NoEscape(f'{np.abs(val):.02f}'
                                f'\\angle{np.angle(val, deg=True):.02f}')
            else:
                return NoEscape(f'{np.abs(val):.04f}')
        else:
            return NoEscape('$\\infty$')
    else:
        if np.abs(val) < 1e3:
            return f'{np.abs(val):6.02f}'
        else:
            if np.abs(val) < 1e6:
                return f'{np.abs(val):6.02E}'.replace('+0', '')
            else:
...
def text_report(system, curves, grid, filename):
    ...
    f.write('======================\n\n')
    f.write('Bus TPG            SLG            DLG'
            '                           LL\n')
    f.write('    Ia(pu) \u03B4a(deg) Ia(pu) \u03B4a(deg) Ib(pu) '
            '\u03B4b(deg) Ic(pu) \u03B4c(deg) Ib(pu) \u03B4b(deg)\n')
    for b in buses:
        f.write(f'{b.bus_id + 1:3d} '
                f'{safe_repr(b.iTPG, is_tex=False)} '
                f'{np.angle(b.iTPG, deg=True):7.02f} '
                f'{safe_repr(b.iSLG, is_tex=False)} '
                f'{np.angle(b.iSLG, deg=True):7.02f} '
                f'{safe_repr(b.iDLGb, is_tex=False)} '
                f'{np.angle(b.iDLGb, deg=True):7.02f} '
                f'{safe_repr(b.iDLGc, is_tex=False)} '
                f'{np.angle(b.iDLGc, deg=True):7.02f} '
                f'{safe_repr(b.iLL, is_tex=False)} '
